...
class FileUtil:
    ...
    def scan_directory(self, inDIR):
        ...
        for dName, sdName, fList in os.walk(inDIR):
            ...
            for fileName in fList:
                original_file_listing.append(os.path.join(dName, fileName))
                #if fnmatch.fnmatch(fileName, unixpattern):
                matches = re.match(repattern, fileName)
                #print("m: ", m)
                if matches:
                    original_file_listing_selected.append(os.path.join(dName, fileName))
        ...
        print("Number of duplicate IDs found:", dupecount, "files.")
    ...
    def read_file_listing(self, file):
        ...
        #TODO: ignore this function if filename matches FILELIST (gfyfetch_filelist.txt)? No need to redo this.
    ...
    def remove_first_line(self, file):
        ...
        #FIXME: check if sed exists like below and use check_call after checking file isfile
        cmd = ['sed', '-i', '-e', "1d", file]
